...
def init_deepspeed(args, model, optimizer, lr_scheduler):
    ...
    ds_config = get_train_ds_config(
        offload=args.offload, dtype=args.dtype, stage=args.zero_stage, args=args
    )
...
def get_args_parser():
    ...
    parser.add_argument(
        "--dataset",
        default="CSL_Daily",
        choices=["CSL_News", "CSL_Daily", "WLASL", "BOBSL", "How2Sign"],
    ...
    parser.add_argument(
        ...
        type=str,
        choices=["stgcn", "mlp"],
        help="The type of encoder to use for processing pose data. "
        "Options are 'stgcn' or 'mlp'. "
        "This will determine how the pose data is processed and interpreted. "
        "Ensure that the encoder type is compatible with the pose data format and the overall model architecture. "
        "For example, 'stgcn' is suitable for spatio-temporal graph convolutional networks, "
        "while 'mlp' is suitable for multi-layer perceptrons to compare encoded pose features. ",
    )
